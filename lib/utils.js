...
exports.request = function(method, path, options) {
  const create_defaults = exports.defaults();
  const opts = Object.assign({}, create_defaults(method, path, options));
  let data = /^(put|patch|post)$/i.test(method) ? options : null;
  let json = data ? JSON.stringify(sanitize(data, opts.params)) : null;
  ...
  switch (verb) {
    case 'post':
      promise = instance.post(url, json, opts);
      break;
    case 'put':
      promise = instance.put(url, json, opts);
      break;
    case 'patch':
      promise = instance.patch(url, json, opts);
      break;
    case 'delete':
  ...
  return promise.then(res => {
    ...
    return result(github_res, json, opts, options);
  });
};
...
exports.defaults = function (appOptions = null) {
  ...
  return function(method, uri, methodOptions) {
    const options = Object.assign({}, defaults, appOptions, methodOptions);
...
function result(res, data, opts, options) {
  ...
  const url = res.url || opts.url || '';
  if (/\/markdown\/raw/.test(url) && opts.headers && opts.headers['content-type'] === 'text/plain') {
    try {
