...
exports.request = function(method, path, options) {
  ...
  switch (verb) {
    case 'post':
      promise = instance.post(opts.url, data, opts);
      break;
    case 'put':
      promise = instance.put(opts.url, data, opts);
      break;
    case 'patch':
      promise = instance.patch(opts.url, data, opts);
      break;
    case 'delete':
      promise = instance.delete(opts.url, data, opts);
      break;
    case 'get':
    default:
      promise = instance.get(opts.url, data, opts);
      break;
  }
...
exports.paged = function(method, path, options) {
   ...
   *
   * @param {string} _url - The URL of the current page
   * @param {GitHubXHR} res - The response object
   * @param {Array} acc - The accumulated response data
   * @returns {string?}
   */
  function nextPage(_url, res, acc) {
    ...
    let page = res.headers.link ? parseLink(res.headers.link) : null;
    return page && page.next ? page.next.url : null;
  }
...
function result(res, data, opts, options) {
  ...
  if (/\/markdown\/raw/.test(res.url) && opts.headers && opts.headers['content-type'] === 'text/plain') {
    try {
