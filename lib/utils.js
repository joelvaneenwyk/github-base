 ...
 * @module utils
 *
 * @typedef {Axios.IPromise<Axios.AxiosXHR<any>>} AxiosResponse
 * @typedef {(url: string, res: any, acc: any[]) => void} Next
 * @typedef {Axios.AxiosXHRConfigBase & {
 *   method?: string,
 *   url?: string,
 *   next?: Next,
 *   bearer?: string,
 ...
 * @param {Options} options - The request options including headers, params, etc.
 * @returns
 */
exports.request = function(method, path, options) {
  ...
  /** @type {Axios.AxiosXHRConfigBase} */
  const config = {};
  let instance = axios.create(config);
  let verb = (opts.method || '').toLowerCase();

  /** @type {Axios.IPromise<Axios.AxiosXHR<any>>} */
  let promise;
  let url = opts.url || '';
  switch (verb) {
    case 'post':
  ...
  return promise.then(res => {
    res.body = res.data;
    return result(res, data_string, opts, options);
  });
};
 ...
 * @return {(method: string, uri: string, methodOptions: Options) => Options} Returns a function that takes a request method name, the URL to expand, and method options.
 */
